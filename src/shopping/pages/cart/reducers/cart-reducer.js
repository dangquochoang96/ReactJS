...
export const cartReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case types.CHANGE_QTY:
      ...
      const newListCard = state.dataCart.map(item => {
        return item.id === idCart ? { ...item, qty: qtyCart } : item 
      })

      //cap nhat tong tien
      // duyet lai du lieu cart + cong don tien lai
      const newTotalMoney = newListCard.map(item => parseInt(item.price) * parseInt(item.qty)).reduce((pre, next) => pre + next);
      return {
        ...state,
        errorCart: null,
        dataCart: newListCard,
